...
export function calculateGrade(results: HeaderResult[], options: GradeOptions = {}): GradeResult {
  ...
  const sriRawMaxScore = Math.max(0, Math.trunc(options.sriMaxScore ?? 0));
  const sriRawScore = Math.max(0, Math.min(Math.trunc(options.sriScore ?? 0), sriRawMaxScore));
  const securityTxtRawMaxScore = Math.max(0, Math.trunc(options.securityTxtMaxScore ?? 0));
  const securityTxtRawScore = Math.max(
    0,
  ...
  const mixedContentRawMaxScore = Math.max(0, Math.trunc(options.mixedContentMaxScore ?? 0));
  const mixedContentRawScore = Math.max(
    0,
    Math.min(Math.trunc(options.mixedContentScore ?? 0), mixedContentRawMaxScore)
  );
  const dnssecRawMaxScore = Math.max(0, Math.trunc(options.dnssecMaxScore ?? 0));
